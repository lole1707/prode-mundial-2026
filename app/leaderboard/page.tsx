        ...
        {users.length === 0 ? (
          ...
          <div className="space-y-2">
            {users.map((u, i) => (
              <div
                key={u.uid}
                className={`flex items-center gap-4 bg-gray-900 border rounded-xl px-4 py-3 ${u.uid === user?.uid ? "border-green-700" : "border-gray-800"}`}
              >
                <span className="w-8 text-center text-lg font-bold">
                  {i < 3 ? medals[i] : <span className="text-gray-500 text-sm">#{i + 1}</span>}
                </span>
                <div className="flex-1">
                  <p className="font-semibold">{u.displayName} {u.uid === user?.uid && <span className="text-xs text-green-400">(vos)</span>}</p>
                  <p className="text-xs text-gray-500">{u.email}</p>
                </div>
                <div className="text-right">
                  <p className="text-2xl font-bold text-green-400">{u.totalPoints}</p>
